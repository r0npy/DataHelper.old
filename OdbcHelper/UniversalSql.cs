...
namespace Universal.Data
{
    ...
    internal class UniversalSql : DbDataAccess
    {
        ...
        {
            return new OdbcCommand(sqlCommand)
            {
                CommandType = commandType,
                Connection = (OdbcConnection)Conexion
            };
        }
        ...
        {
            var da = new OdbcDataAdapter((OdbcCommand)Comando(sqlCommand, commandType));
            if (args.Length != 0)
        ...
        protected override IDataAdapter CrearDataAdapter(IDbTransaction transaction, string sqlCommand,
            CommandType commandType, params IDbDataParameter[] args)
        {
            var da = new OdbcDataAdapter(((OdbcCommand)Comando(transaction, sqlCommand, commandType)));
            if (args.Length != 0)
                CargarParametros(da.SelectCommand, args);
            return da;
        }
